...
import { useIo } from "@/provider/io.provider";
...
export default function StartPage() {
  ...
  const handelQr = (qr: string) => setQr(qr);
  useEffect(() => {
    if (!io) return;
    io.on("qr", handelQr);
  ...
  return (
    <div>
      <div className="max-w-md min-h-160 mx-auto bg-card border-[#FFFFFF1A] p-8  border mt-10 rounded-[32px] card-shadow relative overflow-hidden">
        ...
        <div className="mt-10">
          {qr ? (
            <img className=" mx-auto rounded-md  w-1/2" src={qr} alt="" />
          ) : (
            <div className="size-32 mx-auto flex justify-center items-center  flex-col text-center">
